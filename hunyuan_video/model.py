...
class NAGHunyuanVideo(HunyuanVideo):
    ...
    def forward(
        ...
        if apply_nag:
            ...
            if attention_mask is not None and isinstance(attention_mask, torch.Tensor) and attention_mask.ndim > 0 and attention_mask.shape[0] == pos_bsz:
                if nag_bsz > pos_bsz:
                    repeat_times = (nag_bsz + pos_bsz - 1) // pos_bsz
                    m_neg = attention_mask.repeat(repeat_times, *[1]*(attention_mask.ndim-1))[:nag_bsz]
                else:
                    m_neg = attention_mask[:nag_bsz]
                attention_mask = torch.cat([attention_mask, m_neg], dim=0)
